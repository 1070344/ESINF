...
void Ficheiro:: LerFicheiroLocais(){
	string nomef1="Ficheiro1.txt";	
	ifstream ler(nomef1.c_str()); // Verifica se o Ficheiro existe
	if (!ler) 
		cout << "Erro na leitura do ficheiros dos locais turisticos!"<<endl<<endl;			
	while (ler.getline(linha,1024)!=NULL){	// Fazer o Split de cada linha pela ',' para criação do objecto 
		cout << "ola" <<endl;
		cont++;
		token=strtok(linha,",");
		string chave = token;	
		token = strtok (NULL, ",");
		string designa = token;
		token = strtok (NULL, ",");
		double req = atof(token);	
		//Edificio* e= new Loja(chave,designa,req); 	// Criar Edificio Loja
		//edificio.insere(cont,new Loja(chave,designa,req));
		//edificio.insere(cont,e); // Inserir na lista ligada
	}
	ler.close();
}
